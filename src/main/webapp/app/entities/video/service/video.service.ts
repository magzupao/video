...
type RestOf<T extends IVideo | NewVideo> = Omit<T, 'fechaCreacion' | 'fechaDescarga'> & {
...
export class VideoService {
  ...
  partialUpdate(video: PartialUpdateVideo): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(video);
    return this.http
      .patch<RestVideo>(`${this.resourceUrl}/${encodeURIComponent(this.getVideoIdentifier(video))}`, copy, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<RestVideo>(`${this.resourceUrl}/${encodeURIComponent(id)}`, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  protected convertDateFromClient<T extends IVideo | NewVideo | PartialUpdateVideo>(video: T): RestOf<T> {
    return {
      ...video,
      fechaCreacion: video.fechaCreacion?.toJSON() ?? null,
      fechaDescarga: video.fechaDescarga?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restVideo: RestVideo): IVideo {
    return {
      ...restVideo,
      fechaCreacion: restVideo.fechaCreacion ? dayjs(restVideo.fechaCreacion) : undefined,
      fechaDescarga: restVideo.fechaDescarga ? dayjs(restVideo.fechaDescarga) : undefined,
    };
  }
